...
pbxProject.prototype.removeFromPbxHeadersBuildPhase = function(file) {
    var sources = this.pbxHeadersBuildPhaseObj(file.target), i;
...
function pbxFileReferenceObj(file) {
    var fileObject = {
        ...
        name: sanitizeFilename(file.basename),
        path: sanitizePathname(file.path),
        sourceTree: file.sourceTree,
    };

    if (file.explicitFileType)
        fileObject.explicitFileType = file.explicitFileType;

    if (file.includeInIndex)
        fileObject.includeInIndex = file.includeInIndex;
